...
and writes PNGs to docs/assets/:
  ...
  - docs/assets/perf_speedup.png   — FastDOCX speedup factor over python-docx
...
FASTDOCX_COLOR = "#581aa8"
DOCX_COLOR = "#1aa897"
FASTDOCX_SCATTER = "#9361d4"
DOCX_SCATTER = "#75cbc1"
...
def load_results() -> tuple[list[int], list[float], list[float]]:
    ...
    fast_times = [data["fastdocx"][str(s)] * 1000 for s in sizes]
...
def plot_time(sizes: list[int], docx_times: list[float], fast_times: list[float]) -> None:
    ...
    ax.bar(x - bar_w / 2, fast_times, bar_w, label="FastDOCX", color=FASTDOCX_COLOR, zorder=3)
    ...
    ax.set_title("Build + save time: FastDOCX vs python-docx")
...
def plot_speedup(sizes: list[int], docx_times: list[float], fast_times: list[float]) -> None:
    ...
    ax.set_title("FastDOCX speedup over python-docx")
...
def plot_size() -> None:
    fastdocx_mb = _dir_size_mb(_package_root("fastdocx"))
    pydocx_mb = _dir_size_mb(_package_root("docx"))

    print(f"  fastdocx   : {fastdocx_mb:.2f} MB")
    ...
    bars = ax.bar(
        ...
        [fastdocx_mb, pydocx_mb],
        color=[FASTDOCX_COLOR, DOCX_COLOR],
        width=0.45,
        zorder=3,
    )
    for bar, size in zip(bars, [fastdocx_mb, pydocx_mb]):
        ax.text(
            bar.get_x() + bar.get_width() / 2,
            bar.get_height() + max(fastdocx_mb, pydocx_mb) * 0.01,
            f"{size:.2f} MB",
            ha="center",
            va="bottom",
            fontsize=10,
